...
describe('signIn', () => {
    it('should sign in with returned token', async () => {
        firebase.auth.signInWithCustomToken = jest.fn(() => {});
        ...
        return store.dispatch(await authActions.signIn({cprNumber, code})).then(() => {
            expect(store.getActions()).toEqual(expectedAction)
            expect(firebase.auth.signInWithCustomToken).toHaveBeenCalledTimes(1);
            expect(firebase.auth.signInWithCustomToken).toHaveBeenCalledWith();
